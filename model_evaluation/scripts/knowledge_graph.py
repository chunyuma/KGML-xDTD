...
class KnowledgeGraph(nn.Module):
    ...
    def vectorize_action_space(self):
        ...
        def vectorize_action_space(action_space_list, action_space_size):
            ...
            if self.use_gpu:
                r_space = eval_utilities.int_var_cuda(r_space, args=self.args, use_gpu=True)
                e_space = eval_utilities.int_var_cuda(e_space, args=self.args, use_gpu=True)
                action_mask = eval_utilities.var_cuda(action_mask, args=self.args, use_gpu=True)
            else:
                r_space = eval_utilities.int_var_cuda(r_space, args=self.args, use_gpu=False)
                e_space = eval_utilities.int_var_cuda(e_space, args=self.args, use_gpu=False)
                action_mask = eval_utilities.var_cuda(action_mask, args=self.args, use_gpu=False)     
    ...
    def define_modules(self):
        ...
        entity_embeds = eval_utilities.entity_load_embed(self.args)
        ...
        relation_embeds = eval_utilities.relation_load_embed(self.args)
    ...
    @property
    def self_edge(self):
        return eval_utilities.SELF_LOOP_RELATION_ID

    @property
    def dummy_r(self):
        return eval_utilities.DUMMY_RELATION_ID

    @property
    def dummy_e(self):
        return eval_utilities.DUMMY_ENTITY_ID
