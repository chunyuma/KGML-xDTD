...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_name", type=str, help="log file name", default="step8_logistic_2class.log")
    ...
    ## create model folder
    folder_name = 'logistic_model_2class'
    ...
    lg_model = LogisticRegression(class_weight='balanced', random_state=args.seed, max_iter=5000, n_jobs=-1)
    param_grid = { 'penalty' : ['l1', 'l2', 'elasticnet']}
    gs_lg = GridSearchCV(estimator=lg_model, param_grid=param_grid, cv= 10, scoring='f1_macro', return_train_score=True)
    gs_lg.fit(train_X, train_y)
    logger.info(f"The best hyper-parameter set of logistic model based on gridseaerchCV is {gs_lg.best_estimator_}")
    best_lg = gs_lg.best_estimator_
    ...
    with open(os.path.join(args.output_folder, folder_name, 'grid_search_cv_results.pkl'), 'wb') as outfile:
        pickle.dump(gs_lg.cv_results_, outfile)
    fitModel = best_lg.fit(train_X, train_y)

    # # saves the model
    model_name = f'logistic_model.pt'
    ...
    with open(os.path.join(args.output_folder, folder_name, f'logistic_results.pkl'), 'wb') as outfile:
