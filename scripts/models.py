...
class PreActorCritic(ActorCritic):
    def __init__(self, args, kg, history_len, gamma, target_update, hidden_sizes):
        super(PreActorCritic, self).__init__(args, kg, history_len, gamma, target_update, hidden_sizes, True)

    def batch_get_true(self, batch_action_spaces, batch_true_actions):
    ...
    def select_true_action(self, batch_state, batch_action_spaces, batch_true_actions, device):

        state_inputs = self.process_state(self.history_len, batch_state).to(device)
        true_idx_in_actor, true_next_actions = self.batch_get_true(batch_action_spaces, batch_true_actions)

        probs, q_actions = self.policy_net(state_inputs, batch_action_spaces)

        true_idx_in_actor = true_idx_in_actor.to(device)
        true_prob = probs.gather(1, true_idx_in_actor.view(-1, 1)).view(-1)
        q_act = q_actions.gather(1, true_idx_in_actor.view(-1, 1)).view(-1)

        self.saved_output = SavedOutput(probs, [], torch.log(true_prob+torch.tensor(utils.TINY_VALUE)), q_act, [])
        return true_next_actions

    def imi_step_update(self, next_batch_state, batch_action_spaces, batch_reward, done, optimizer, device, update):
        critic_loss = torch.tensor([0.0])
        actor_loss = torch.tensor([0.0])
        if update == 'critic':
            # update critic
            curr_reward = torch.tensor(batch_reward).to(device)
            curr_q_act = self.saved_output.q_action
            if done is False:
                next_state_inputs = self.process_state(self.history_len, next_batch_state).to(device)
                next_target_probs, next_target_q_actions = self.target_net(next_state_inputs, batch_action_spaces)
                y = curr_reward + self.gamma * torch.sum(torch.mul(next_target_probs, next_target_q_actions), dim=1).view(-1)
            else:
                y = curr_reward
            critic_loss = (curr_q_act - y).pow(2).mean()
            optimizer.zero_grad()
            ...
            # update actor
            log_prob_action = self.saved_output.log_prob_action
            loss = -log_prob_action
            loss = loss.mean()
            if torch.isnan(loss):
                self.args.logger.error(f"{self.saved_output.actor_probs}")
            actor_loss = loss
            optimizer.zero_grad()
            actor_loss.backward()
            optimizer.step()
