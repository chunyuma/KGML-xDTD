...
class GAT(torch.nn.Module):
    def __init__(self, args, in_channels, hidden_channels, out_channels, num_layers, dropout_p, num_head = 8, bias=True):
        ...
        self.transform_lin = torch.nn.Linear(in_channels, hidden_channels, bias=bias)
        for _ in range(num_layers):
            self.convs.append(GATConv(hidden_channels, hidden_channels, heads=num_head, dropout=dropout_p, concat=False, bias=bias))
        self.lin1 = torch.nn.Linear(hidden_channels*2,hidden_channels,bias=bias)
        self.lin2 = torch.nn.Linear(hidden_channels,out_channels,bias=bias)
    ...
    def forward(self, init_mat, adjs, link, n_id, return_attention=False):
        ...
        for i, (edge_index, size) in enumerate(adjs):
            ...
            x_target = x[:size[1]].to(self.args.device)
